...
pragma solidity >=0.6.0 >=0.6.9 ^0.8.0 ^0.8.1 ^0.8.2 ^0.8.20 ^0.8.24;
...
interface IERC20 {
    /**
     * @dev Emitted when `value` toke           ns are moved from one account (`from`) to
     * another (`to`).
     *
...
abstract contract Initializable {
    ...
    function _setInitializedVersion(uint8 version) private returns (bool) {
        ...
        if (_initializing) {
            require(
                version == 1 && !Address.isContract(address(this)), "Initializable: contract is already initialized"
            );
            return false;
        } else {
